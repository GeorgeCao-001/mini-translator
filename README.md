...
- **可视化进度窗**：应用内弹窗，**标题栏可拖动**；✕ / ESC / 点击外部 = **最小化成右下角悬浮球**（玻璃质感圆球 + 进度环，可拖动、方向键微调，翻译继续跑，最小化期间 Obsidian 完全可正常操作），点击悬浮球（位移 <5px）弹回完整进度窗；动态进度条带高光流动动画和百分比标签，平滑过渡；显示总进度条、当前阶段（提取→翻译→渲染→装配）、当前文件与批次、已完成块数、≈tokens 已用、ETA 剩余时间，一键取消
- **悬浮球换肤**：设置里可选五款皮肤——水墨晕染 / 星云斜带 / 双层潮汐 / 琥珀对流 / 静态冰棱，浅深主题自动适配；拖动位置与所选皮肤自动记忆

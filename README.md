...
- **可视化进度窗**：应用内弹窗，**标题栏可拖动**；✕ / ESC / 点击外部 = **最小化成右下角极光悬浮球**（旋转渐变内核 + 呼吸彩色光晕 + 渐变进度环，可拖动，翻译继续跑，最小化期间 Obsidian 完全可正常操作），点击悬浮球弹回完整进度窗；动态进度条带高光流动动画和百分比标签，平滑过渡；显示总进度条、当前阶段（提取→翻译→渲染→装配）、当前文件与批次、已完成块数、≈tokens 已用、ETA 剩余时间，一键取消

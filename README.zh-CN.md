...
在侧边栏或插件设置中展开翻译源／词典源下拉列表，可**直接拖动选项排序**；第一项为首选源，点击某项也会将其设为首选。内置源旁的开关控制其能否作为备用源（首选源始终会尝试）。不支持当前语言对的内置翻译源会跳过；大模型配置仅能作为首选源，不会暗中切换到其他服务。

### 大模型服务

插件支持用户自行配置 OpenAI 兼容的 Chat Completions 接口，并提供 DeepSeek、OpenAI、Google Gemini、xAI、Mistral AI、Groq、OpenRouter、Kimi、通义千问、智谱 GLM、硅基流动、Ollama、LM Studio 和 vLLM 等配置模板。模板只预填接口，不再内置容易过期的模型名称，也不提供账号、额度或 API Key。

填写接口所需的 API Key 后，点击 **从接口获取模型**，再从接口实际返回的模型 ID 中选择翻译模型。成功获取后会用本次真实返回结果替换该配置原来的发现列表，避免继续保留已下线或过期的预设名称；若兼容网关没有实现 `/models`，仍可手动填写其文档给出的模型 ID。
...
| 查询模型 | 根据用户配置的接口推导并请求 `/models`，用实际返回的 ID 更新该配置的模型列表 |
